...
matched_pairs = [(index_T, index_B) for index_T, center_T in enumerate(centers_T) for index_B, center_B in enumerate(centers_B) if np.linalg.norm(np.subtract(center_T, center_B)) <= 8]
...
for (index_T, index_B) in matched_pairs:
    ...
    if not skip_pair:
        t_img = images_to_save[0]
        t_img = cv2.cvtColor(t_img,cv2.COLOR_BGR2RGB)
        b_img = images_to_save[1]
